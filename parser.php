...
final class Image {
  ...
  public function __construct($src, $fileName) {
    $this->src = $src;
...
final class Parser {
  ...
  public function setFile($path) {
    $this
      ...
      $this->docx_file = \PhpOffice\PhpWord\IOFactory::load($path);
  }
  /**
        ...
        foreach ($body as $prop => $elem) {
          ...
          if ('p' === $prop) {
            ...
            if ($elem->pPr->pStyle) {
              ...
              // echo var_dump($objectStyle);

              if (preg_match('/Titre/', $objectStyle)){
                $isHeading = TRUE;
              }
            ...
            if ($elem->pPr->spacing) {
              $att = $elem->pPr->spacing->attributes('w', TRUE);
            ...
            if ($elem->pPr->numPr && !$isHeading) {
              ...
              if ($openList) {
                ...
                $listTags = $this->getListFormatting($elem->pPr->numPr);
              ...
              if (!$closeList) {
                ...
                if ($elem->pPr->jc) {
                  $att = $elem->pPr->jc->attributes('w', TRUE);
            ...
            if ($isHeading) {
              if (preg_match('/1/', $objectStyle)) {
                  $html .= '<h1>';
              } elseif (preg_match('/2/', $objectStyle)) {
                  $html .= '<h2>';
              } elseif (preg_match('/3/', $objectStyle)) {
                  $html .= '<h3>';
              }
              /* Add other conditions for H4,H5,H6 */
              elseif (preg_match('/4/', $objectStyle)) {
                  $html .= '<h4>';
              } elseif (preg_match('/5/', $objectStyle)) {
                  $html .= '<h5>';
              } elseif (preg_match('/6/', $objectStyle)) {
                  $html .= '<h6>';
              }
              /* Then add the parsed text*/
              $html .= $this->parseText($elem, $startTags, $startAttrs);
              /* for H1, for H2 use </h2> and so on*/
              if (preg_match('/1/', $objectStyle)) {
                  $html .= '</h1>';
              } elseif (preg_match('/2/', $objectStyle)) {
                  $html .= '</h2>';
          ...
          }elseif ('r'/*c'est le format pour les images, texte et liens */  === $prop && $elem->pict) {
            echo "yes";
            $html.= '<img src="' . $this->parseImage($elem, $startTags, $startAttrs ) . '">';
        } else {
              /* add the parsed text if it's not a heading */
              $html .= $this->parseText($elem, $startTags, $startAttrs);
          }
...
/**
 * Parse un élément image et renvoie la chaîne HTML correspondante.
 *
 * @param SimpleXMLElement $elem L'élément à parser.
 * @param array $tags Tableau des balises HTML à appliquer à l'image.
 * @param array $attrs Tableau des attributs CSS à appliquer à l'image.
 * @return string La chaîne HTML.
 */
private function parseImage($elem, array $tags = [], array $attrs = []) {
    
  // On récupère l'attribut "embed" de l'élément "blip" dans l'élément "blipFill".
  $attr = $elem->pict->blipFill->blip->attributes('r', TRUE);
  $target = $attr['embed'];

  // On récupère la liste des relations du document.
  $rels = $this->docx_file->relationships();
  
  // On récupère l'image correspondante à la relation.
  $image = $rels->getTarget($target);
  
  // On récupère le contenu.
  $imageData = $image->getContent();

  // On détermine l'extension de l'image en fonction de son type (png etc, tu connais).
  $extension = $this->getImageExtension($image->getContentType());

  // On génère un nom de fichier unique pour l'image.
  $filename = uniqid() . $extension;

  // On crée le fichier temporaire et on y écrit les données binaires de l'image.
  $filepath = sys_get_temp_dir() . DIRECTORY_SEPARATOR . $filename;
  file_put_contents($filepath, $imageData);
  
  // On construit les balises HTML à partir des tags et attributs fournis.
  $openTags = '';
  $closeTags = '';
  ...
  // On construit la chaîne HTML finale avec l'image et les balises HTML.
  $html = '<span style="' . implode(';', $attrs) . '">' . $openTags . '<img src="' . $filepath . '" />' . $closeTags . '</span>';

  return $html;
}

protected function getImageExtension($contentType) {
    switch ($contentType) {
        case 'image/png':
            return '.png';
        case 'image/gif':
